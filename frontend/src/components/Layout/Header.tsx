...
import type { HeaderData, HeaderNavLink } from '@/types/api';
...
const hasNavChildren = (link: HeaderNavLink) =>
...
export const Header = ({ header }: HeaderProps) => {
  ...
  const [openMobileAccordionId, setOpenMobileAccordionId] = useState<
    string | null
  >(null);

  const openMegaMenuLink = header.navLinks.find(
    (link) => link.id === openMegaMenuId
  );

  const closeMegaMenu = () => setOpenMegaMenuId(null);
        ...
        >
            ...
            >
              {header.navLinks.map((link) =>
                hasNavChildren(link) ? (
                  <div
                    key={link.id}
                    onMouseEnter={() => setOpenMegaMenuId(link.id)}
                  >
                    <Link
                      href={link.href}
                      variant="nav"
                      aria-expanded={openMegaMenuId === link.id}
                      aria-haspopup="true"
                      className={cn(openMegaMenuId === link.id && 'text-accent')}
                      onClick={handleNavClick}
                    >
                      {link.label}
                    </Link>
          ...
          {openMegaMenuLink?.children && (
            <div
              ...
              aria-hidden={!openMegaMenuId}
              onMouseEnter={() => setOpenMegaMenuId(openMegaMenuLink.id)}
            >
              <NavMegaMenu
                items={openMegaMenuLink.children}
                onNavigate={handleNavClick}
                ariaLabel={openMegaMenuLink.label}
              />
            </div>
          )}
        </div>
              ...
              <li key={link.id}>
                <button
                  ...
                  className={cn(
                    ...
                    openMobileAccordionId === link.id
                      ? 'text-accent'
                      : 'text-white/80 hover:text-accent'
                  )}
                  onClick={() =>
                    setOpenMobileAccordionId((current) =>
                      current === link.id ? null : link.id
                    )
                  }
                  aria-expanded={openMobileAccordionId === link.id}
                  aria-controls={`mobile-nav-children-${link.id}`}
                >
                  {link.label}
                  <ChevronDown
                    size={18}
                    className={cn(
                      ...
                      openMobileAccordionId === link.id && 'rotate-180'
                    )}
                    aria-hidden="true"
                  />
                </button>
                ...
                <div
                  id={`mobile-nav-children-${link.id}`}
                  className={cn(
                  ...
                  aria-hidden={openMobileAccordionId !== link.id}
                >
